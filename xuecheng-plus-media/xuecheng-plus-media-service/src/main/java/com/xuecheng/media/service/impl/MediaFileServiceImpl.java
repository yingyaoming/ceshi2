...
//    }
    ...
    */
     ...
     * @return
     */
    public MediaFiles uploadFileNew(MultipartFile fileData) {
        //获取扩展名
        String originalFilename = fileData.getOriginalFilename();
        String extension = originalFilename.substring(originalFilename.lastIndexOf("."));

        String fileFolder = getFileFolder(new Date(), true, true, true);
        //获取MD5
        String MD5;
        try {
            MD5 = DigestUtils.md5Hex(fileData.getInputStream());
            log.info("源文件MD5：{}",MD5);
        } catch (IOException e) {
            e.printStackTrace();
            log.error("获取MD5失败{}",e.getStackTrace());
            throw new RuntimeException(e);
        }

        String object =fileFolder + MD5 + extension;
        //构建临时文件
        File tempFile;
        try {
        ...
        try {
            UploadObjectArgs uploadObjectArgs = UploadObjectArgs.builder()
                    .bucket("mediafiles")
                    .object(object)
                    .filename(tempFile.getAbsolutePath())//源文件地址
                    .build();
            minioClient.uploadObject(uploadObjectArgs);
        } catch (Exception e) {
            log.error("文件路径错误：{}",e);
        ...
        mediaFiles.setCompanyName("哈哈乐园");
        mediaFiles.setFilename(object);
        mediaFiles.setFileType("jpg");
        mediaFiles.setBucket("mediafiles");
        mediaFiles.setFilePath(fileFolder);
        mediaFiles.setFileId("11111111111");

        try {
            mediaFilesMapper.insert(mediaFiles);
        }catch (Exception e){
            throw new XueChengPlusException("插入错误");
        }
